        ...
        *,
        ...
        if global_dim < dim:
            ...
            kmeans, centroids, self.cluster_map = run_kmeans(
                reduced_train_np, n_clusters, euclidean, balanced=True
            )
            centroid_train_map: Sequence[np.ndarray] = kmeans.assign(reduced_train_np, train_size)
        else:
            if isinstance(data, np.ndarray):
                kmeans, centroids, self.cluster_map = run_kmeans(
                    data, n_clusters, euclidean, balanced=True
                )
                centroid_train_map = kmeans.assign(data, train_size)
            else:
                kmeans, centroids, self.cluster_map = run_kmeans(
                    data_t.cpu().numpy(), n_clusters, euclidean, True
                )
        ...
        for start in range(0, n_clusters, batch_size):
            ...
            pts_batched = torch.zeros(B_sz, m_max, dim, dtype=dtype, device=device)
            q_batched = torch.zeros(B_sz, l_max, dim, dtype=dtype, device=device)
            ...
            for j, cid in enumerate(batch_ids):
                ...
                B_pad = torch.zeros(rank, self.max_cluster_size, dtype=dtype, device=device)
                B_pad[:, :m_i] = V[j, :m_i].T
                self.B.append(B_pad)

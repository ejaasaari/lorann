...
class LorannBase {
 public:
  LorannBase(T *data, int m, int d, int n_clusters, int global_dim, int rank, int train_size,
             Distance distance, bool balanced)
      : _data(data),
        _n_samples(m),
        _dim(d),
    ...
    if (d % 8 != 0) {
      throw std::invalid_argument("LoRANN requires the dimensionality to be a multiple of 8.");
    }
   ...
   */
  inline float get_dissimilarity(const T *u, const T *v) {
   ...
   */
  void exact_search(const T *q, int k, int *out, float *dist_out = nullptr) const {
    Vector dist(_n_samples);

    const T *data_ptr = _data;
    const int width = _dim / detail::Traits<T>::dim_divisor;
 ...
 protected:
  ...
  void save(Archive &ar) const {
    ...
    ar(_n_samples);
    ar(_dim);
    ar(cereal::binary_data(_data, sizeof(T) * _n_samples * width), _n_clusters, _global_dim,
       _max_rank, _train_size, static_cast<int>(_distance), _balanced, _cluster_map,
       _global_centroid_norms);
  }
  ...
  void load(Archive &ar) {
    ...
    _owned_data = std::vector<T>(_n_samples * width);
    ...
    if (_distance == L2) {
      _data_norms = Vector(_n_samples);
      for (int i = 0; i < _n_samples; ++i) {
        _data_norms(i) =
            detail::Traits<T>::dot_product(_data + i * width, _data + i * width, width);
      }
    }
  }

  T *_data;
  std::vector<T> _owned_data;

...
class FullRangeFitter {
  static constexpr std::array<double, 8> kBoundaryInverse =
      {2., 2./3., 2./5., 2./7., 2./9., 2./11., 2./13., 2./15.};
 public:
  static int encode(float value, float factor) {
  ...
  float fit(const float *values, int size) {
    ...
    for (int iteration = 0; iteration < 3; ++iteration) {
      ...
      if (error < best_loss) { best_loss = error; best = initial; }
    }
    ...
    auto consider = [&](double ideal) {
      ...
      for (float factor : {rounded, std::nextafter(rounded, 0.f),
        ...
        if (!std::isfinite(factor) || std::abs(factor) < ColumnFitter<4>::kMinFactor ||
            std::abs(factor) > ColumnFitter<4>::kMaxFactor) continue;
        const double candidate = stored_loss(values, size, factor);
        if (candidate < best_loss) { best_loss = candidate; best = factor; }
      }
    };
    ordered_.clear();
    double energy = 0;
    ...
    std::sort(ordered_.begin(), ordered_.end(), [](const Value &a, const Value &b) {
      return a.x < b.x;
    });
    for (int sign : {1, -1}) {
      if (sign < 0)
      ...
      for (size_t i = 0; i < values_.size(); ++i) {
        ...
        xx += v.x*v.x; xk += v.x*v.limit; kk += v.limit*v.limit;
        if (xx <= best_error) continue;
        const double root = (xx-best_error) /
            (xk+std::sqrt(std::max(0., xk*xk-kk*(xx-best_error))));
      ...
      for (int level = 0; level < 8; ++level) {
        ...
        size_t index = std::upper_bound(ordered_.begin(), ordered_.end(), lower,
            [inverse](double bound, const Value &v) { return bound < v.x*inverse; }) - ordered_.begin();
        if (level == 7)
      ...
      for (;;) {
        ...
        if (norm > 0 && dot >= lower*norm && dot <= upper*norm &&
          ...
          if (error < best_error) {
            best_error = error; best_scale = scale; improved = true;
          }
        }
        if (heap_size == 0 || zero_energy >= best_error) break;
        Stream next = heap[0];
        const double x = ordered_[next.index].x;
        dot -= x; norm -= 2*next.level+1;
        lower = next.scale;
        if (next.level == 0) zero_energy += x*x;
        ++next.index;
        if (next.level == 7)
 ...
 private:
  struct Value { double x, saturation; int limit; };
  struct Stream { double scale; size_t index; int level; };
...
inline void refit_correction(const ColMatrix &a, const ColMatrix &b,
                             const ColMatrixUInt8 &qa, ColMatrixUInt8 &qb,
                             const Vector &ca, Vector &cb) {
  ...
  const Eigen::MatrixXd reduced = gram.bottomRightCorner(rank,rank)-cross*cross.transpose()/norm;
  const double diagonal_threshold = 1e-12 * reduced.trace();
  Eigen::VectorXd z(rank), best(rank), rhs(rank), gradient(rank), reduced_z(rank), decoded(rank+1),
      gram_decoded(rank+1);
  for (int j = 0; j < b.cols(); ++j) {
    ...
    auto retain = [&]() {
      ...
      if (!std::isfinite(factor) || std::abs(factor) < ColumnFitter<4>::kMinFactor ||
          std::abs(factor) > ColumnFitter<4>::kMaxFactor) return;
      ...
      if (error < best_loss) {
        best_loss = error; best = z; best_factor = factor; best_correction = correction;
      }
    };
    retain();
    ...
    for (int iteration = 0; iteration < 4; ++iteration) {
      ...
      for (int i = 0; i < rank; ++i) {
        ...
        const double code = std::clamp(std::floor(z[i]-gradient[i]/reduced(i,i)+.5),-8.,7.);
    ...
    cb[j] = best_factor; cb[b.cols()+j] = best_correction;
